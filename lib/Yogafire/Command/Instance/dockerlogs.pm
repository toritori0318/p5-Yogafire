package Yogafire::Command::Instance::dockerlogs;
use Mouse;
...
has 'docker-logs-tail' => (
    traits        => [qw(Getopt)],
    isa           => "Bool",
    is            => "rw",
    cmd_aliases   => "dt",
    documentation => "Docker logs tail flag.",
);
no Mouse;

use Yogafire::CommandClass::InstanceProc;

sub abstract {'EC2 Docker logs'}

sub command_names {'docker-logs'}

sub usage {
    my ( $self, $opt, $args ) = @_;
    $self->{usage}->{leader_text} = 'yoga docker-logs [-?] <tagsname>';
    $self->{usage};
}
