package Yogafire::Command::Instance::dockerexec;
use Mouse;
...
has 'docker-exec-command' => (
    traits        => [qw(Getopt)],
    isa           => "Str",
    is            => "rw",
    cmd_aliases   => "dc",
    documentation => "Docker exec command. (default: /bin/bash)",
);
no Mouse;

use Yogafire::CommandClass::InstanceProc;

sub abstract {'EC2 Docker exec'}

sub command_names {'docker-exec'}

sub usage {
    my ( $self, $opt, $args ) = @_;
    $self->{usage}->{leader_text} = 'yoga docker-exec [-?] <tagsname>';
    $self->{usage};
}
...
sub execute {
    ...
    $opt->{"docker_command"} = "exec";
    $opt->{"docker-exec-command"} ||= "/bin/bash";
